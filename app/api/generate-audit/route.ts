...
export async function POST(req: NextRequest) {
  try {
    ...
    const toolContextLines = allSelectedTools.map((toolName) => {
      ...
      return `${toolName}: unrecognized tool — assess manually based on category and typical pricing`;
    });
...
{
  "headline": "string — one punchy sentence summarizing the waste picture",
  ...
  "quick_wins": ["string — up to 5 specific actions they can take immediately"],
  "environmental_note": "string — honest plain-language note about the energy footprint of their stack",
  "redirect_estimate_usd": number
}

Be specific. Name actual tools. Give real dollar estimates based on typical pricing. If you cannot estimate confidently, give a conservative range. Never use vague language like "significant" or "substantial" — use numbers.`;
    ...
    if (auditId && process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      try {
        ...
        // Log but don't fail — report is still returned to client
