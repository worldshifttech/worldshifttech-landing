...
- [x] Session 78 — Two client-facing UX fixes. The Progress card/mini bar (both `app/projects/[slug]/page.tsx` and the client hub list) now hides until a project has at least one milestone — a stalled 0% bar read as broken, not "not started yet." `OpenItems.tsx` can now carry one attached file per submission (new `project_feedback.attached_file_id`, FK to `project_files`) — raising a ticket and attaching a file used to be two disconnected sections with no way to link them; the upload itself reuses the exact signed-URL flow `FileUploads.tsx` already had. The standalone `Files` section is unchanged — still needed for Drew sending files down to a client, not just up. Verified with `npx tsc --noEmit`, `npm run build`, `npx eslint` (caught a real unescaped-apostrophe error before shipping).
- [x] Session 80 — Attach-files control added to Run Planning Session on `/admin/repos/[id]` (control-plane half only): new `session-context-files` storage bucket, `agent_session_context_files` table, `/api/admin-repos/context-files/upload-url`, and `dispatchOrchestratorSession()` now signs and forwards `context_files` in the `repository_dispatch` payload. The matching `wst-orchestrator-runner` consumption side (downloading those URLs into the sandboxed checkout, pointing Claude Code's Read tool at them) is not yet built — see `ORCHESTRATOR_DESIGN.md` §4.
- [x] Session 81 — Open Items now supports client-side edit and delete. A client can edit an open item's message (and attach an additional file) or delete it outright, as long as Drew hasn't marked it resolved yet — new `app/api/project-feedback/[id]/route.ts` (PATCH/DELETE), same dual-actor access pattern as `project-files/[id]`. `project_files.feedback_id` (new) exists alongside `milestone_id` as a second scoping column, letting a file attached during an edit link back to the ticket it was added to.
