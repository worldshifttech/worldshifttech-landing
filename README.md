...
- [x] Session 60 — "Dispatching to: {repo name}" badge on the Run Planning Session and Run Custom Build Session boxes on `/admin/repos/[id]`, after a real dispatch landed against the wrong repo (`wst-build-manager` instead of `worldshifttech-landing`) because both pages look identical apart from the small heading.
...
- [x] Session 62 — Pre-filled a nav-cohesion audit brief into `worldshifttech-landing`'s own Run Planning Session box (component state only, not a new feature/table) — Drew's next planning session once Session 61 is done, sitting ready rather than needing to be retyped.
